...
void LogPrintf( LogType logType, LPCSTR fmt, ... )
{
	va_list marker;
	TCHAR szBuf[LOG_MESSAGE_OUTPUT_BUFFER_SIZE];
...
DWORD LogMaskStr2Mask( const char* maskStr )
{
	int c;
	DWORD mask;
...
void DeleteExpiredLogs( LPCSTR logDir, LPCSTR logFiles, time_t logExpirationPeriod )
{
	time_t			ft;
	time_t			now;
	HANDLE			fh;
